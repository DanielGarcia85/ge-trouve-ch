...
# ── Constantes ────────────────────────────────────────────────────────
# Les réglages (TOP_K, OPTIONS_GENERATION, TIMEOUT_GENERATION) vivent dans config.py ;
# ici ne restent que ce qui relève de la logique du pipeline, pas d'un réglage.
...
def construire_pipeline(consigne=None, options=None, streaming_callback=None):
    ...
    options = options or config.OPTIONS_GENERATION
    ...
    pipe.add_component("retriever", ChromaEmbeddingRetriever(document_store=store, top_k=config.TOP_K))
    pipe.add_component(
        "prompt_builder",
    ...
    pipe.add_component(
        ...
        OllamaChatGenerator(
            ...
            timeout=config.TIMEOUT_GENERATION, generation_kwargs=options,
